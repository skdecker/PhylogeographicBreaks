...
myfiles <- list.files(path ="/Users/decker.391/Documents/Research/Phylogatr/FinalVersions/Data/Aves/", pattern = "_final.fasta", full.names=TRUE, recursive = TRUE)
...
for (f in myratfiles) {
  ...
  write.table(data.frame(sp, f, length), file="/Users/decker.391/Documents/Research/Phylogatr/FinalVersions/Data/Aveslengths.csv", quote=FALSE, row.names=FALSE, col.names=FALSE, append=TRUE, sep=",", na="0")
}

#read in csv with the number of sequences per species and keep proceed with species with >14 sequences
lengths <- read.csv(file="/Users/decker.391/Documents/Research/Phylogatr/FinalVersions/Data/Aveslengths.csv", header=FALSE)
newAvesfiles <- lengths$V2[which(lengths$V3 > 14)]
```

```{r}
#set folder for PDFs of results
PDFFolder <- "/Users/decker.391/Documents/Research/Phylogatr/FinalVersions/Data/Aves/PDFs/"

#loop through species folders to conduct Monmonmier's algorithm analysis
for (i in 1:length(newAvesfiles)) {
  
  #get species and gene name
  sp <- basename(newAvesfiles[195])
  sp <- file_path_sans_ext(sp)
  
  #read in fasta file
  seqs <- read.multiFASTA(files=newAvesfiles[195])
  ...
  write.table(data.frame(sp, Deldir1x, Deldir1y), file="/Users/decker.391/Documents/Research/Phylogatr/FinalVersions/Data/Avesbreaks.csv", quote=FALSE, row.names=FALSE, col.names=FALSE, append=TRUE, sep=",", na="0")
    write.table(data.frame(sp, Deldir2x, Deldir2y), file="/Users/decker.391/Documents/Research/Phylogatr/FinalVersions/Data/Avesbreaks2.csv", quote=FALSE, row.names=FALSE, col.names=FALSE, append=TRUE, sep=",", na="0")
